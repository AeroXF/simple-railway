    ...
    layer.photos = function(a, c, d) {
        function e(a, b, c) {
            ...
            d.onload = function() {
                d.onload = null,
                b(d)
                var sWidth = $("div.layui-layer-photos").width();
                var sHeight = $("div.layui-layer-photos").height();
                var pWidth = $("div.layui-layer-photos").find("img:eq(0)").width();
                var pHeight = $("div.layui-layer-photos").find("img:eq(0)").height();
                if(pWidth > sWidth || pHeight > sHeight){
                	var sRatio = (sHeight/sWidth).toFixed(4);
                	var pRatio = (pHeight/pWidth).toFixed(4);
                	if(sRatio > pRatio){	//以宽度为准
                		$("div.layui-layer-photos").find("img:eq(0)").css("max-width", sWidth);
                		$("div.layui-layer-photos").find("img:eq(0)").css("max-height", sWidth*pRatio);
                	}else{
                		$("div.layui-layer-photos").find("img:eq(0)").css("max-height", sHeight);
                		$("div.layui-layer-photos").find("img:eq(0)").css("max-width", sHeight/pRatio);
                	}
                }
