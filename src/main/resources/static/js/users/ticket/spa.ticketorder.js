...
spa.ticketorder = (function(){
	...
	var  initViews = function(){
		...
		if(trainNo.startsWith("D")){
			var str1 = "<span>" + $("#hidden_queryDate").val() + "(" + mapWeekDay[queryDate.getDay()] + ")</span><span>" + trainNo + "次" + 
	...
	var appendTicketTableRow = function(data){
	...
	var initButtons = function(){
		...
		$("#ticket_submit_btn").click(function(){
			layer.confirm('确认提交订单?', {icon: 3, title:'提交订单'}, function(index){
				...
				$.post(APP_PATH + "/ticketOrder/add/ticketOrder", $(form).serialize(), function(data){
