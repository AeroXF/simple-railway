 ...
 * 
 */
function drawContactsTable(list){
	console.log(list);
	...
	$(".delete-contacts-icon").click(function(){
		...
		layer.confirm('确认要删除所选中的联系人么?', {icon: 3, title:'删除联系人'}, function(index){
			layer.close(index);
			$.get("/contacts/del/" + username, function(list){
		...
		layer.open({
			...
			success: function(layero, index){
				...
				$("#save_add_contact_btn").click(function(){
					...
					$.post("/contacts/modify/" + id, formData, function(list){
						layer.close(index);
...
function initViews(){
	...
	$("#contacts").addClass("model-active").css("padding-left", "26px");
	
	$.get("/contacts/get/contacts", function(list){
		drawContactsTable(list);
	});
}
...
function initButtons(){
	$("#add_contacts_btn").click(function(){
		layer.open({
			...
			success: function(layero, index){
				...
				$("#save_add_contact_btn").click(function(){
					var formData = $("#add_contact_form").serialize();
					$.post("/contacts/add/contact", formData, function(list){
						layer.close(index);
						drawContactsTable(list);
					});
				});
	...
	$("#del_contacts_btn").click(function(){
		...
		layer.confirm('确认要删除所选中的联系人么?', {icon: 3, title:'删除联系人'}, function(index){
			layer.close(index);
			$.get("/contacts/del/" + userArray.join("-"), function(list){
				layer.alert('删除联系人成功', {title:'删除联系人'});
				drawContactsTable(list);
			});
		});
